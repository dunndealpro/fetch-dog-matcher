...
const FindMatchButtonFloat: FC<FindMatchButtonFloatProps> = ({ likedDogs,setLikedDogs, className, showFindMatchButtonFloat }) => {
...
if(likedDogs?.likedDogs){
  ...
  return (
    <>    
      ...
      <Button className={`find-match-button-float fixed-bottom ${!buttonActive ? 'invisible' : ''}`} onClick={findMatch}  disabled={!buttonActive}>Get matched!</Button>
      <MatchModal
        show={matchModal}
        match={match}
        onHide={handleClose}
